...
function Mood(props) {
    ...
    useEffect(() => {
      function GetRepresentMoodList() {
        return axios.get(`http://localhost:5050/represent_mood_list`)
        .then((element) => {
                ...
                <div id="Mood__navbar--list" className="Mood__navbar--list">{
                    RepresentMoodList.map((represent_mood) => (
                        <div id={represent_mood?.image_name} className="Mood__navbar--item" onClick={SelectMood}>
                            <img className="Mood__item--image" src={`http://localhost:5050/assets/Mood_Type/${represent_mood?.image_name}.svg`}/>
